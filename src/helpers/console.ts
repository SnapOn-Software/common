...
export const getStackCallersDefaultFilter = () => ['node_modules', 'node:internal'];
...
export function getStackCallers(levels?: number, skip?: number, filter?: string[]) {
    filter = isNotEmptyArray(filter) ? filter : getStackCallersDefaultFilter();
    ...
    const stack = allLines
        ...
        .filter(s => !firstOrNull(filter, f => s.includes(f)))
        .reverse();
